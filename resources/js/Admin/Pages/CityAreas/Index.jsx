...
import CreateCityArea from "../../Components/Dashboard/CityArea/CreateCityArea";
import EditCityArea from "../../Components/Dashboard/CityArea/EditCityArea";

export default function Index(props) {

    const {data: cityAreas, meta} = props.cityAreas;
    const cities = props.cities;
    const [state, setState] = useState([])
    const [addDialogHandler, addCloseTrigger,addTrigger] = useDialog()
    const [UpdateDialogHandler, UpdateCloseTrigger,UpdateTrigger] = useDialog()
    const [destroyDialogHandler, destroyCloseTrigger,destroyTrigger] = useDialog()
    const openUpdateDialog = (cityArea) => {
        setState(cityArea);
        UpdateDialogHandler()
    }

    const openDestroyDialog = (cityArea) => {
        setState(cityArea);
    ...
    const destroyUser = () => {
        Inertia.delete(
            route('city-areas.destroy', state.id),
    ...
    return (
        <>
            <div className="container-fluid py-4">
                <Dialog trigger={addTrigger} title={__('Create New City Area')}>
                    <CreateCityArea cities={cities} close={addCloseTrigger}/>
                </Dialog>

                <Dialog trigger={UpdateTrigger} title={`${__('Update City Area')}: ${state.id}`}>
                    <EditCityArea model={state} cities={cities} close={UpdateCloseTrigger}/>
                </Dialog>

                <Dialog trigger={destroyTrigger} title={`Delete City Area: ${state.id}`}>
                    <p>{__('Are you sure to delete this City Area ?')}</p>
                                ...
                                <table className="table align-items-center justify-content-center mb-0" width="100%">
                                    <thead>
                                        <tr>
                                            ...
                                            <th className="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 text-center">{__('City')}</th>
                                    ...
                                    <tbody>
                                        {cityAreas.map((cityArea, index) => (
                                            <tr key={cityArea.id}>
                                                <td className='text-center'>{meta.from + index}</td>
                                                <td className='text-center'>{cityArea.city_title}</td>
                                                <td className='text-left'>
                                                        <Tabs className="mb-3">
                                                            {
                                                                cityArea.translations.map(({locale,title}) => (
                                                ...
                                                <div>
                                                    <button type="button" onClick={() => openUpdateDialog(cityArea)} className="btn btn-vimeo btn-icon-only mx-2">
                                                        <span className="btn-inner--icon"><i className="fas fa-pencil-alt"/></span>
                                                    </button>
                                                    <button type="button" onClick={() => openDestroyDialog(cityArea)} className="btn btn-youtube btn-icon-only">
...
Index.layout = (page) => <Base key={page} children={page} title={__('Manage City Areas')}/>
